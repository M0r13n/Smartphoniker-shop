...
class TestMerge:
    ...
    def test_name(self, db, app, sample_repair, sample_device, some_devices):
    ...
    def test_image(self, db, app, sample_device, some_devices):
        merger = Device.merge([sample_device.id, ] + list(map(lambda d: d.id, some_devices)))
        assert merger.image == sample_device.image

    def test_is_tablet(self, db, app, sample_device, some_devices):
        merger = Device.merge([sample_device.id, ] + list(map(lambda d: d.id, some_devices)))
        assert merger.is_tablet == sample_device.is_tablet

    def test_repairs(self, db, app, sample_device, sample_repair, another_repair, another_device):
    ...
    def test_colors(self, db, app, sample_device, another_device, sample_color):
    ...
    def test_delete(self, db, app, sample_device, some_devices):
